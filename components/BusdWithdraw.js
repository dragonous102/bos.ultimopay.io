...
function BtcWithdraw({wallet , id , user , twofa}) {
    ...
    const balanceIncFee = bal > 0 ? truncateFloat((bal - fee) / 1.001, 6) : 0;
    ...
    function onSubmit(user) {
      ...
      {
        if(balanceIncFee < parseFloat(inputValue))
        {
          console.log(balanceIncFee);
      ...
      return Swal.fire({
        title: `Do you really want to Withdraw ${inputValue} ${id} ?`,
  ...
  <div className="row pe-4 ps-5 pt-5 pb-5">
    ...
    <p>
        <b>IMPORTANT NOTICE</b><hr/>
        <p>
        BOS supports only BUSD withdrawals: BNB Smart Chain (BEP20). Please carefully check the network for your
        destination address (the address you will withdraw to).
        </p>
        <p> <b>Network :</b> BNB Smart Chain (BEP20)</p>
      </p>
      <table>
        <tbody>
          <tr>
          ...
          <tr>
            ...
            <td>{balanceIncFee.toFixed(float)} {id} </td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      <h4 className="mt-5">
        ...
        <div className="mb-3">
          <input
            ...
            placeholder={`Enter ${wallet.title} address`}
          />
